...
const Fact = ({
  ...
  <div
    className={`flex justify-between py-0.5 border-t border-gray-400 ${indent ? 'pl-4' : ''} ${isPreview ? 'text-gray-500' : ''}`}
  >
  ...
  return (
    <div className="w-64 p-2 bg-white text-black border-2 border-black font-sans text-sm rounded-md">
      ...
      {previewMacros.fat > 0 && (
        <Fact
          label="Preview Fat"
          value={`+${Math.round(previewMacros.fat)}g`}
          indent
          isPreview
        />
      )}
      ...
      <Fact
        ...
        isBold
      />
      {previewMacros.carbs > 0 && (
        <Fact
          label="Preview Carbs"
          value={`+${Math.round(previewMacros.carbs)}g`}
          indent
          isPreview
        />
      )}
      ...
      {previewMacros.protein > 0 && (
        <Fact
          label="Preview Protein"
          value={`+${Math.round(previewMacros.protein)}g`}
          indent
          isPreview
        />
      )}
                ...
                <TooltipTrigger asChild>
                  <div className="z-10 flex flex-col h-full w-1/6 items-center cursor-pointer">
                    <div className="relative w-8 h-full bg-gray-200/50 rounded-t-lg overflow-hidden flex flex-col-reverse">
                      ...
                      <MacroBar
                        macro="Fat"
                        value={previewData.fat}
                        goal={goals.fat}
                        color="#facc15"
                        opacity={0.5}
                      />
                    </div>
                    <p className="text-xs mt-1 text-center font-semibold text-white mix-blend-difference">
                      {meal}
                    </p>
                    <p className="text-xs text-white mix-blend-difference">
                      {Math.round(mealData.calories)} cal
                    </p>
                  </div>
                </TooltipTrigger>
                <TooltipContent>
