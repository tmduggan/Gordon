...
import { muscleMapping, specialMuscleGroups } from '../../../utils/muscleMapping';
...
export default function MuscleChart({ className = "", onMuscleClick }) {
  ...
  const { normalizedScores, rawScores, extraScores, specialScores } = useMemo(() => {
    ...
    Object.entries(specialMuscleGroups).forEach(([specialGroup, libraryMuscles]) => {
      let todayScore = 0;
      let score3day = 0;
      let score7day = 0;
  ...
  return (
    <div className={`w-full aspect-[5/3] relative ${className}`} ref={containerRef}>
      {/* SVG Muscle Chart */}
        ...
        {Object.entries(combinedMuscles).map(([id, paths]) => {
          ...
          return (
            <g
              key={id}
              id={id}
              onMouseEnter={isInteractive ? (e => handleMuscleHover(id, e)) : undefined}
            ...
            >
              {paths.map((d, i) => (
                <path
                  key={i}
                  d={d}
                  fill={isBackground ? '#f0f0f0' : getMuscleColor(normalizedScores[id])}
                  stroke="#333"
                  strokeWidth="2"
                />
              ))}
