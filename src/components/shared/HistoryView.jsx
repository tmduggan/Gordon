                    ...
                    if (group.type === 'recipe') {
                      ...
                      return (
                        <React.Fragment key={group.groupKey}>
                          <tr
                            className="border-b bg-orange-50 cursor-pointer"
                            onClick={() =>
                              setExpandedRecipe(
                                expandedRecipe === group.groupKey
                                  ? null
                                  : group.groupKey
                              )
                            }
                          >
                            <td className="font-semibold flex items-center gap-2">
                              🍲 {group.recipeName} (
                              {group.recipeLoggedServings} serving
                              {group.recipeLoggedServings > 1 ? 's' : ''})
                              <Button
                                variant="ghost"
                                size="icon"
                                className="h-6 w-6 ml-2"
                                title="Delete entire recipe log"
                                onClick={(e) => {
                                  e.stopPropagation();
                                  group.logs.forEach((l) => deleteLog(l.id));
                                }}
                              >
                                <X className="h-4 w-4 text-red-500" />
                              </Button>
            ...
            <tbody>
              {dateLogs.map((log) => (
                <ExerciseLogRow
                  ...
                  log={log}
                  getExerciseName={(id) =>
                    getExerciseName(id, exerciseLibrary?.items || [])
                  }
                  deleteLog={deleteLog}
                />
              ))}
