import { doc, setDoc, getDocs, collection, query, where, orderBy, getDoc, updateDoc } from 'firebase/firestore';
...
export async function rejectExerciseSubmission(submissionId, adminUserId, adminNotes = '') {
  try {
    const submissionRef = doc(db, 'exerciseSubmissions', submissionId);
    await updateDoc(submissionRef, {
      status: 'rejected',
