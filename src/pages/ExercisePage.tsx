...
export default function ExercisePage() {
  ...
  const availableEquipment = ensureAvailableEquipment(
    userProfile?.availableEquipment
  );
  let selectedEquipment = [];
  if (selectedFilter === 'bodyweight')
    selectedEquipment = availableEquipment.bodyweight;
  else if (selectedFilter === 'gym') selectedEquipment = availableEquipment.gym;
  else if (selectedFilter === 'cardio')
    selectedEquipment = availableEquipment.cardio;
  else
    selectedEquipment = [
      ...new Set([
  ...
  const laggingMuscles = React.useMemo(() => {
    if (!userProfile || !exerciseHistory.logs || !exerciseLibrary.items)
      return [];
    return analyzeLaggingMuscles(
        ...
        equipmentButtons={
          <div className="flex justify-center gap-2 mb-4">
            <button
              ...
              onClick={() =>
                setSelectedFilter(
                  selectedFilter === 'bodyweight' ? null : 'bodyweight'
                )
              }
            >
              Body Weight
            ...
            <button
              ...
              onClick={() =>
                setSelectedFilter(selectedFilter === 'gym' ? null : 'gym')
              }
            >
              Gym Equipment
            </button>
            <button
              ...
              onClick={() =>
                setSelectedFilter(selectedFilter === 'cardio' ? null : 'cardio')
              }
            >
              Cardio
            </button>
